...
%% 100Hz模型
Fn = 100;
[XTrain,YTrain,XVal,YVal,XTest,YTest,N_EIS,eis_mean_std,SOC_T] = lstm_data_set_onehot_half(Fn,1);
load(['lstm_ui2eis_battery_half_',num2str(Fn),'.mat']);
lstm_model_eis_plot(net,trainInfo,XTest,YTest,N_EIS,eis_mean_std,SOC_T);gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP1-LSTM-EIS-HALF-',num2str(Fn),'Hz'],'-dsvg','-r1200');
eis_mean_std1 = eis_mean_std;

[XTrain,YTrain,XVal,YVal,XTest,YTest,N_EIS,eis_mean_std,SOC_T] = lstm_data_set_onehot_half(Fn,3);
load(['lstm_ui2eis_battery_half_',num2str(Fn),'.mat']);
lstm_model_eis_plot(net,trainInfo,XTest,YTest,N_EIS,eis_mean_std,SOC_T);gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP3-LSTM-EIS-HALF-',num2str(Fn),'Hz'],'-dsvg','-r1200');
...
for i=1:1:5
    Fn = F(i);
    load(['lstm_ui2eis_battery_half_',num2str(Fn),'.mat']);
    [XTrain1,YTrain1,XVal1,YVal1,XTest1,YTest1,N_EIS,eis_mean_std1,SOC_T1] = lstm_data_set_onehot_half(Fn,1);
    [XTrain3,YTrain3,XVal3,YVal3,XTest3,YTest3,N_EIS,eis_mean_std3,SOC_T3] = lstm_data_set_onehot_half(Fn,3);
    %%% RMSE
    ...
    print(gcf, ['Figs/LFP-Real-Imag-Compare Half ',num2str(i)],'-dsvg','-r1200');
...
figure,
set(gcf, 'Position',[0 0 480 480],'Color','White');
colormap(color(1:51,:));         % 使用自定义 colormap
cb = colorbar;           % 生成 colorbar
cb.Ticks = linspace(0,1,10);
cb.TickLabels = 1:10;    % 可按需要修改
...
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP-HALF-RMSE-Real-Train'],'-dsvg','-r1200');
...
axis([10^-1.5 10^4.5 0.00 11.9])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP-HALF-RMSE-Real-Val'],'-dsvg','-r1200');
...
axis([10^-1.5 10^4.5 0.00 11.9])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP-HALF-RMSE-Real-Test'],'-dsvg','-r1200');
...
axis([10^-1.5 10^4.5 0.00 11.9])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP-HALF-RMSE-Imag-Train'],'-dsvg','-r1200');
...
axis([10^-1.5 10^4.5 0.00 11.9])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP-HALF-RMSE-Imag-Val'],'-dsvg','-r1200');
...
axis([10^-1.5 10^4.5 0.00 11.9])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP-HALF-RMSE-Imag-Test'],'-dsvg','-r1200');
%% 实部
for i=1:1:5
    for j=1:1:5
...
axis([10^-1.5 10^4.5 0.00 11.9])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP-HALF-RMSE-Imag-100Hz'],'-dsvg','-r1200');
...
set(gca,'xtick',10.^[-1:2:4],'ytick',-2000:0.5:2000,'FontSize',24,'LabelFontSizeMultiplier',1,'FontName','Arial');

axis([10^-1.5 10^4.5 0.00 2.9])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP-HALF-MAPE-AM'],'-dsvg','-r1200');
...
set(leg,'Location','Northwest','FontSize',24,'FontName','Arial','Orientation','v','NumColumns',1,'box','off');
leg.ItemTokenSize=[60,18];  %
xlabel("Frequency (Hz)");ylabel('RMSE of Phase Angle \rm(\circ)');ytickformat('%.1f')
set (gca,'LineWidth',3,'position',[0.24,0.26,0.74,0.7]);
set(gca,'xtick',10.^[-1:2:4],'ytick',-2000:0.5:2000,'FontSize',24,'LabelFontSizeMultiplier',1,'FontName','Arial');

axis([10^-1.5 10^4.5 0.00 2.9])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP-HALF-MAPE-Phase'],'-dsvg','-r1200');

SCSER_MAPE = aveRMSE;
aveRMSE_Z = aveRMSE_real;
save SCSER_Error.mat SCSER_MAPE aveRMSE_Z
%% 不同模型RMSE battery1
freq = 10.^[4:-0.1:-1];
F = [100 50 25 10 5];
for i=1:1:5
...
set(gca,'xtick',10.^[-1:2:4],'ytick',0:3:9,'FontSize',24,'LabelFontSizeMultiplier',1,'FontName','Arial');
axis([10^-1.5 10^4.5 0.00 13.9]);
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP1-HALF-RMSE-Real-Hz'],'-dsvg','-r1200');
...
axis([10^-1.5 10^4.5 0.00 17.9])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP1-HALF-RMSE-Imag-Hz'],'-dsvg','-r1200');
...
axis([10^-1.5 10^4.5 0.00 13.9]);
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP3-HALF-RMSE-Real-Hz'],'-dsvg','-r1200');
...
axis([10^-1.5 10^4.5 0.00 17.9])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP3-HALF-RMSE-Imag-Hz'],'-dsvg','-r1200');
%%
figure,
color =  colorInterp_lan(6, 8);
...
set(gca,'xtick',0:100:500,'ytick',0:0.5:2000,'FontSize',32,'LabelFontSizeMultiplier',1.2,'FontName','Arial');
axis([-10 510 0.00 1.88])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP HALF LOSS'],'-dsvg','-r1200');
...
save EIS_Half_pre.mat EIS_imag EIS_real T0 SOC0 EIS_real0 EIS_imag0
...
for i=1:1:5
    Fn = F(i);
    load(['lstm_ui2eis_battery_half_',num2str(Fn),'.mat']);
    [X1,Y1,eis_mean_std1,SOC1,Temp1,N_EIS] = lstm_data_set_onehot_half_all(Fn,1);
    [X3,Y3,eis_mean_std3,SOC3,Temp3,N_EIS] = lstm_data_set_onehot_half_all(Fn,3);
    X =[X1;X3];Y =[Y1;Y3];SOC=[SOC1;SOC3];Temp = [Temp1;Temp3];
    %%% RMSE
    YPred1 = predict(net,X1);YPred3 = predict(net,X3);
end
save EIS_Half_pre4temp_soc.mat Y1 Y3 YPred1 YPred3 eis_mean_std1 eis_mean_std3 SOC1 Temp1 SOC3 Temp3
...
%% 本地函数 2：全部样本版本（用于汇总）
%% ========================================================================
function [X,Y,eis_mean_std,SOC,Temp,N_EIS] = lstm_data_set_onehot_half_all(Fn,Number)
