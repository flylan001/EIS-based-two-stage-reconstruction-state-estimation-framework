...
%% 100Hz模型
Fn = 100;
[XTrain,YTrain,XVal,YVal,XTest,YTest,N_EIS,eis_mean_std,SOC_T] = lstm_data_set_no_onehot_half(100,1:51);
...
print(gcf, ['Figs/LFP100-half'],'-dsvg','-r1200');
%% 50Hz模型
Fn = 50;
[XTrain,YTrain,XVal,YVal,XTest,YTest,N_EIS,eis_mean_std,SOC_T] = lstm_data_set_no_onehot_half(100,11:41);
load(['lstm_ui2eis_battery_half_',num2str(Fn),'.mat']);
YPred = predict(net,XTest);   % 预测第1个样本
figure,
set(gcf, 'Position',[0 0 600 600],'Color','White');
...
set(gca,'xtick',-1000:2000:5000,'ytick',-1000:2000:2000,'FontSize',32,'LabelFontSizeMultiplier',1.2,'FontName','Arial');
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP50-half'],'-dsvg','-r1200');
%% 模型RMSE
freq = 10.^[4:-0.1:-1];
freq1 = 10.^[3:-0.1:0];
F = [100 50 25 10 5];
...
for i=1
    Fn = F(i);
    load(['lstm_ui2eis_battery_half_',num2str(Fn),'.mat']);
    [XTrain,YTrain,XVal,YVal,XTest,YTest,N_EIS,eis_mean_std,SOC_T] = lstm_data_set_no_onehot_half(100,index{i} );
...
for i=1
    Fn = F(i+1);
    load(['lstm_ui2eis_battery_half_',num2str(Fn),'.mat']);
    [XTrain,YTrain,XVal,YVal,XTest,YTest,N_EIS,eis_mean_std,SOC_T] = lstm_data_set_no_onehot_half(100,index{i+1} );
    EIS_real_pre = [];EIS_imag_pre = [];
    %%% RMSE
    YPred = predict(net,XTrain);
    EIS_exp_real = [(YTrain(:,1:N_EIS)*eis_mean_std(1,2)+eis_mean_std(1,1) )*1000000;];
...
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP-MAPE-AM-half'],'-dsvg','-r1200');
...
axis([10^-1.5 10^4.5 0.00 4.46])
gca=boxoff(gca);clear gca;
print(gcf, ['Figs/LFP-MAPE-Phase-half'],'-dsvg','-r1200');
...
print(gcf, ['Figs/LFP-AM-Phase 100-half 10000-01'],'-dsvg','-r1200');
...
print(gcf, ['Figs/LFP-AM-Phase 100-half 1000-1'],'-dsvg','-r1200');


function [XTrain,YTrain,XVal,YVal,XTest,YTest,N_EIS,eis_mean_std,SOC_T] = lstm_data_set_no_onehot_half(Fn,Number)
